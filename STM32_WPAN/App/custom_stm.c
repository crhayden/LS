...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomLodestar_SHdle;                    /**< Lodestar_l handle */
  uint16_t  CustomLs_Char_SHdle;                  /**< LS_Char_l handle */
...
#define COPY_LODESTAR_L_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xab,0xcd,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_LS_CHAR_L_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xab,0xce,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomLs_Char_SHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for Lodestar_l +
   *                                2 for LS_Char_l +
   *                              = 3
   *
  ...
  COPY_LODESTAR_L_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomLodestar_SHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Lodestar_s, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Lodestar_s \n\r");
  }

  /**
   *  LS_Char_l
   */
  COPY_LS_CHAR_L_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomLodestar_SHdle,
                          UUID_TYPE_128, &uuid,
                          SizeLs_Char_S,
                          ...
                          &(CustomContext.CustomLs_Char_SHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : LS_CHAR_S, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : LS_CHAR_S \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LS_CHAR_S:
      ret = aci_gatt_update_char_value(CustomContext.CustomLodestar_SHdle,
                                       CustomContext.CustomLs_Char_SHdle,
                                       0, /* charValOffset */
                                       SizeLs_Char_S, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value LS_CHAR_S command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value LS_CHAR_S command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LS_CHAR_S:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomLodestar_SHdle, CustomContext.CustomLs_Char_SHdle, SizeLs_Char_S, pPayload);
